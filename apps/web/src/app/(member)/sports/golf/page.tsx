...
async function fetchGolfEvents(): Promise<GolfEvent[]> {
  const response = await apiGet<GolfEvent[]>('/api/sports/golf/events');
...
export default function GolfPage() {
  ...
  const query = useQuery({
    queryKey: ['sports', 'golf', 'events'],
    queryFn: fetchGolfEvents,
    ...
    <main className="min-h-screen bg-[#0B0E1A] px-4 pb-24 pt-5 text-white md:px-6 md:pb-8">
      <div className="mx-auto max-w-6xl">
        <header className="mb-4 rounded-xl border border-[#1E293B] bg-[#111827] p-4">
          <h1 className="text-2xl font-bold">Golf</h1>
          <p className="text-sm text-[#94A3B8]">Browse live and upcoming golf tournaments.</p>
        </header>

        {query.isLoading ? (
          <div className="rounded-xl border border-[#1E293B] bg-[#111827] p-4 text-[#94A3B8]">Loading golf tournaments...</div>
        ) : null}
        ...
        {!query.isLoading && !query.error && (query.data || []).length === 0 ? (
          <div className="rounded-xl border border-dashed border-[#1E293B] bg-[#111827] p-8 text-center text-[#94A3B8]">
            No golf tournaments available right now. Check back soon.
          </div>
        ) : null}

        <div className="grid gap-3 md:grid-cols-2">
          {(query.data || []).map((event) => {
            const title = event.tournamentName || event.name || event.league || 'Golf Tournament';
            const venue = event.course || event.venue || '';
            ...
            return (
              <article key={event.id} className="rounded-xl border border-[#1E293B] bg-[#1A2235] p-4">
                <div className="flex items-center justify-between text-xs text-[#94A3B8]">
                  <span>{event.league || 'PGA Tour'}</span>
                  <span>{event.status === 'live' ? 'LIVE' : event.status || 'scheduled'}</span>
                </div>
                <Link href={`/sports/golf/${event.id}`} className="mt-2 block text-sm font-semibold text-white hover:text-[#00C37B]">
                  {title}
                </Link>
                {venue && <div className="mt-1 text-xs text-[#94A3B8]">{venue}</div>}
                {event.round && <div className="text-xs text-[#64748B]">{event.round}</div>}
                {!venue && !event.round && (
                    ...
                    return (
                      <button
                        ...
                        onClick={() => togglePick({
                          id: pickId,
                          eventId: event.id,
                          market: market?.name || 'Outright Winner',
                          marketType: market?.type || market?.id || 'tournament_winner',
