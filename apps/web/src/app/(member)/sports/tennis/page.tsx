...
async function fetchTennisEvents(): Promise<TennisEvent[]> {
  const response = await apiGet<TennisEvent[]>('/api/sports/tennis/events');
...
export default function TennisPage() {
  ...
  const query = useQuery({
    queryKey: ['sports', 'tennis', 'events'],
    queryFn: fetchTennisEvents,
    ...
    <main className="min-h-screen bg-[#0B0E1A] px-4 pb-24 pt-5 text-white md:px-6 md:pb-8">
      <div className="mx-auto max-w-6xl">
        <header className="mb-4 rounded-xl border border-[#1E293B] bg-[#111827] p-4">
          <h1 className="text-2xl font-bold">Tennis</h1>
          <p className="text-sm text-[#94A3B8]">Browse live and upcoming tennis matches.</p>
        </header>

        {query.isLoading ? (
          <div className="rounded-xl border border-[#1E293B] bg-[#111827] p-4 text-[#94A3B8]">Loading tennis events...</div>
        ) : null}
        ...
        {!query.isLoading && !query.error && (query.data || []).length === 0 ? (
          <div className="rounded-xl border border-dashed border-[#1E293B] bg-[#111827] p-8 text-center text-[#94A3B8]">
            No tennis events available right now. Check back soon.
          </div>
        ) : null}
            ...
            return (
              <article key={event.id} className="rounded-xl border border-[#1E293B] bg-[#1A2235] p-4">
                <div className="flex items-center justify-between text-xs text-[#94A3B8]">
                  <span>{event.league || 'Tennis'}</span>
                  <span>{event.status || 'scheduled'}</span>
                </div>
                <Link href={`/sports/tennis/${event.id}`} className="mt-2 block text-sm font-semibold text-white hover:text-[#00C37B]">
                  {event.homeTeam?.name || 'Player 1'} vs {event.awayTeam?.name || 'Player 2'}
                </Link>
