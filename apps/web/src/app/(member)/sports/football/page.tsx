...
async function fetchFootballEvents(): Promise<FootballEvent[]> {
  const response = await apiGet<FootballEvent[]>('/api/sports/football/events');
...
export default function FootballPage() {
  ...
  const query = useQuery({
    queryKey: ['sports', 'football', 'events'],
    queryFn: fetchFootballEvents,
  ...
  return (
    <main className="min-h-screen bg-[#0B0E1A] px-4 pb-24 pt-5 text-white md:px-6 md:pb-8">
      <div className="mx-auto max-w-6xl">
        ...
        {query.isLoading ? (
          <div className="rounded-xl border border-[#1E293B] bg-[#111827] p-4 text-[#94A3B8]">Loading football events...</div>
        ) : null}
            ...
            return (
              <article key={event.id} className="rounded-xl border border-[#1E293B] bg-[#1A2235] p-4">
                <div className="flex items-center justify-between text-xs text-[#94A3B8]">
                  <span>{event.league || 'Football'}</span>
                  <span>{event.status || 'scheduled'}</span>
                </div>
                <Link href={`/sports/football/${event.id}`} className="mt-2 block text-sm font-semibold text-white hover:text-[#00C37B]">
