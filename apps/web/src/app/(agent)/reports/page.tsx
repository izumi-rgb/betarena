...
const SPORT_COLORS = ['#00C37B', '#F59E0B', '#3B82F6', '#64748B', '#8B5CF6'];
...
export default function AgentReportsPage() {
  ...
  const SPORTS = [
    { name: 'Football', pct: 85, color: '#00C37B' },
    { name: 'Basketball', pct: 45, color: '#F59E0B' },
    { name: 'Tennis', pct: 30, color: '#3B82F6' },
    { name: 'Cricket', pct: 15, color: '#64748B' },
  ];
  ...
  return (
    <div className="p-10 text-[#F1F5F9]">
      ...
      <div className="mb-8 grid grid-cols-10 gap-6">
        ...
        {/* Bet Type Breakdown Donut */}
        <div className="col-span-4 flex flex-col rounded-xl border border-[#1E293B] bg-[#111827] p-6">
          <h3 className="mb-4 text-[16px] font-bold text-white">Bet Type Breakdown</h3>
          <div className="relative flex flex-1 items-center justify-center">
            <svg width="200" height="200" viewBox="0 0 100 100">
              <circle cx="50" cy="50" r="40" fill="transparent" stroke="#00C37B" strokeWidth="12" strokeDasharray="125 251" transform="rotate(-90 50 50)" />
              <circle cx="50" cy="50" r="40" fill="transparent" stroke="#F59E0B" strokeWidth="12" strokeDasharray="97 251" transform="rotate(90 50 50)" />
              <circle cx="50" cy="50" r="40" fill="transparent" stroke="#3B82F6" strokeWidth="12" strokeDasharray="33 251" transform="rotate(220 50 50)" />
              <circle cx="50" cy="50" r="40" fill="transparent" stroke="#64748B" strokeWidth="12" strokeDasharray="22 251" transform="rotate(265 50 50)" />
            </svg>
            <div className="absolute text-center">
              <div className="text-[10px] font-bold uppercase text-[#64748B]">Total</div>
              <div className="text-[18px] font-bold text-white">{members.length}</div>
              <div className="text-[10px] text-[#64748B]">members</div>
            </div>
          </div>
          <div className="mt-4 grid grid-cols-2 gap-2 text-[12px]">
            {[
              { label: 'Singles', pct: '45%', color: 'bg-[#00C37B]' },
              { label: 'Accas', pct: '35%', color: 'bg-[#F59E0B]' },
              { label: 'System', pct: '12%', color: 'bg-[#3B82F6]' },
              { label: 'Other', pct: '8%', color: 'bg-[#64748B]' },
            ].map((item) => (
              <div key={item.label} className="flex items-center gap-2">
                <div className={`h-2 w-2 rounded-full ${item.color}`} />
                <span className="text-[#94A3B8]">{item.label}</span>
                <span className="ml-auto font-bold text-white">{item.pct}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="mb-8 overflow-hidden rounded-xl border border-[#1E293B] bg-[#1A2235] shadow-lg">
        ...
        <table className="w-full border-collapse text-left">
          ...
          <tbody className="text-[13px] text-[#94A3B8]">
            {filtered.length === 0 ? (
              <tr><td colSpan={7} className="px-6 py-6 text-center text-[#64748B]">No members found.</td></tr>
            ) : (
              filtered.map((m) => (
                <tr
                  key={m.id}
                  className={`group border-b border-[#1E293B] transition-colors last:border-0 hover:bg-[#232d42] ${
                ...
                >
                  ...
                  <td className="px-6 py-4 text-right">
                    <a
                      href={`/members/${m.id}`}
                      className="rounded border border-[#00C37B] px-2 py-1 text-[11px] font-bold text-[#00C37B] opacity-0 transition-all hover:bg-[#00C37B] hover:text-black group-hover:opacity-100"
                    >
                      View Full Profile →
                    </a>
                  </td>
      ...
      {/* Bottom row: Sports Volume + Daily Volume */}
      <div className="grid grid-cols-2 gap-6">
        <div className="rounded-xl border border-[#1E293B] bg-[#1A2235] p-6">
          <h3 className="mb-4 text-[15px] font-bold text-white">Top Sports by Volume</h3>
          <div className="space-y-4">
            {SPORTS.map((sport) => (
              <div key={sport.name}>
                <div className="mb-1 flex justify-between text-[12px]">
                  <span className="text-white">{sport.name}</span>
                  <span className="font-mono" style={{ color: sport.color }}>—</span>
                </div>
                <div className="h-2 w-full overflow-hidden rounded-full bg-[#111827]">
                  <div className="h-full rounded-full" style={{ width: `${sport.pct}%`, backgroundColor: sport.color }} />
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="flex flex-col rounded-xl border border-[#1E293B] bg-[#1A2235] p-6">
          <h3 className="mb-4 text-[15px] font-bold text-white">Daily Bet Volume (14d)</h3>
          <div className="flex flex-1 items-end justify-between gap-2 px-2" style={{ height: '128px' }}>
            {[30, 45, 35, 60, 50, 75, 65, 40, 55, 70, 85, 60, 45, 90].map((h, i) => (
              <div
                key={i}
                className="w-full rounded-t-sm bg-[#00C37B] opacity-60 transition-opacity hover:opacity-80"
                style={{
                  height: `${h}%`,
                  ...(i === 13 ? { opacity: 1, boxShadow: '0 0 10px #00C37B' } : {}),
                }}
                title={i === 13 ? 'Today' : `${14 - i}d ago`}
              />
            ))}
          </div>
