        ...
        <div className="fixed inset-0 z-50 bg-black/50 md:hidden" onClick={closeMobile}>
          <div className="absolute bottom-0 left-0 right-0 max-h-[75vh] overflow-y-auto rounded-t-2xl border-t border-[#1E293B] bg-[#111827] p-4" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-2 pb-36">
              {picks.length === 0 ? (
                <div className="rounded-lg border border-dashed border-[#1E293B] p-4 text-center text-sm text-[#64748B]">
                  Select odds to add bet picks
                </div>
              ) : picks.map((p) => (
            ...
            <div className="fixed bottom-0 left-0 right-0 border-t border-[#1E293B] bg-[#111827] p-4 md:hidden">
              ...
              <button
                disabled={isPlacing || picks.length === 0 || !enoughBalance}
                onClick={placeBet}
