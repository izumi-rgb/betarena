...
const LoginCard = ({ isError, initialUsername, initialPassword, stateLabel }) => {
  const [username, setUsername] = useState(initialUsername || '');
  const [password, setPassword] = useState(initialPassword || '');
  const [showPassword, setShowPassword] = useState(false);
  const [rememberMe, setRememberMe] = useState(isError ? false : true);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '12px' }}>
      <div style={{ fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.1em', color: customStyles.textTertiary, fontWeight: 600, fontFamily: "'Inter', sans-serif" }}>
        {stateLabel}
      </div>
      <div style={{
        width: '400px',
    ...
    <div style={{
      ...
      <div style={{
        ...
        <LoginCard
          ...
          isError={false}
          initialUsername="alex_punter88"
          initialPassword="password123"
        />
        <LoginCard
          stateLabel="Error State"
          isError={true}
          initialUsername="unknown_user"
